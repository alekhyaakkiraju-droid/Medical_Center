...
namespace AngularApi.Validators {
    public class CreateMedicalCenterDTOValidator : AbstractValidator<CreateMedicalCenterDTO> {
        public CreateMedicalCenterDTOValidator() {             RuleFor(x => x.HospitalAffiliationId).GreaterThan(0).When(x => x.HospitalAffiliationId.HasValue);
