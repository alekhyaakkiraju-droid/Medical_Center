...
public class PatientReviewServiceTests : IDisposable {
  ...
  public PatientReviewServiceTests(){_context=new MedicalCenterDbContext(new DbContextOptionsBuilder<MedicalCenterDbContext>().UseInMemoryDatabase(Guid.NewGuid().ToString()).Options); _service=new PatientReviewService(_context,new OwnershipValidator(_context),NullLogger<PatientReviewService>.Instance);}
