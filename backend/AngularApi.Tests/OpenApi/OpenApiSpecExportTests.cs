...
public class OpenApiSpecExportTests : IClassFixture<MedicalCenterWebApplicationFactory>
{
    ...
    public OpenApiSpecExportTests(MedicalCenterWebApplicationFactory factory) => _factory = factory;
    [Fact]
    ...
    public async Task SwaggerEndpoint_ContainsExpectedDtoSchemas()
    {
        ...
        foreach (var schema in new[] { "DoctorDTO", "AppointmentDTO", "PatientDTO", "ReviewDTO", "UpdateAppointmentDTO", "RegisterUserDTO", "LogInUserDTO", "UpdateProfileDto" })
    ...
    public async Task ExportOpenApiSpec_WritesValidJsonWithSchemas()
    {
        ...
        try
        {
            ...
            var response = await client.GetAsync("/swagger/v1/swagger.json");
            response.EnsureSuccessStatusCode();
            await File.WriteAllTextAsync(outputPath, await response.Content.ReadAsStringAsync());
            File.Exists(outputPath).Should().BeTrue();
